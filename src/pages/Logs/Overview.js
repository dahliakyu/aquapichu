import React from 'react'
import userData from 'constants/data'
const ProjectCard = ({ title, link, imgUrl, number }) => {
  return (
    <a href={link} className="w-full block shadow-2xl">
      <div className="relative overflow-hidden">
        <div className="h-72 object-cover">
          <img
            src={imgUrl}
            alt="WIP_IMAGES"
            className="transform hover:scale-125 transition duration-2000 ease-out object-cover h-full w-full"
          />
        </div>
        <h1 className="absolute top-10 left-10 text-gray-50 font-bold text-xl bg-red-500 rounded-md px-2">
          {title}
        </h1>
        <h1 className="absolute bottom-10 left-10 text-gray-900 font-bold text-xl">
          {number.length === 1 ? '0' + number : number}
        </h1>
      </div>
    </a>
  )
}

export default function Projects() {
  return (
    <section className="bg-white dark:bg-stone-800">
      <div className="max-w-6xl mx-auto h-48 bg-white dark:bg-stone-800">
        <h1 className=" dark:text-stone-200 text-5xl md:text-9xl font-bold py-20 text-center md:ml-10 md:text-left">
          Ongoing Experiments
        </h1>
      </div>
      {/* Grid starts here */}
      <div className="bg-[#F1F1F1] dark:bg-stone-900">
        <div className="max-w-6xl mx-auto grid grid-cols-1 gap-8 py-20 md:max-w-6xl md:grid-cols-2 md:py-60 md:pb-80">
          {userData.projects.map((proj, idx) => (
            <ProjectCard
              title={proj.title}
              link={proj.link}
              imgUrl={proj.imgUrl}
              number={`${idx + 1}`}
            />
          ))}
        </div>
      </div>
    </section>
  )
}

